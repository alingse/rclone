...
- |opt| - the option block for the specific serve being configured (optional)
- |vfsOpt| - the option block for the VFS configuration (optional)
- |proxyOpt| for servers which support the auth proxy (optional)

Examples:

    rclone rc serve/start type=nfs fs=remote: opt='{"ListenAddr":":4321"}' vfsOpt='{"CacheMode":"full"}'
    rclone rc serve/start --json '{"type":"nfs","fs":"remote:","opt":{"ListenAddr":":4321"},"vfsOpt":{"CacheMode":"full"}}'
    rclone rc serve/start --json '{"type":"http","fs":"/tmp/nfs","opt":{"HTTP":{"ListenAddr":[":1234"]}},"vfsOpt":{"CacheMode":"off"}}'

The options blocks are as described here

    rclone rc options/info

Eg

    rclone rc --loopback options/info blocks=http

And the default values can be seen with

    rclone rc options/get

Eg

    rclone rc --loopback options/get blocks=http

Note that the |opt| block for a given |type| has the same name as the
|type|, so if |type="sftp"| you should supply |opt| with the |sftp|
options block.
